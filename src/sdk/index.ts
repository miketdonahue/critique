...
interface CritiqueApi {
  queuePrompt(
    note: string,
    options?: Partial<Pick<Prompt, "selector" | "tag" | "text" | "target">>,
  ): void;
  sendQueued(): void;
  end(): void;
...
function applyCardTheme(): void {
  if (shadowRoot)
    (shadowRoot.host as HTMLElement).setAttribute("data-theme", cardTheme);
}

type SdkMessage = SdkToChrome extends infer T
  ? T extends T
    ? Omit<T, "token">
    : never
  : never;
...
function isInteractive(el: Element): boolean {
  ...
  return (
    INTERACTIVE.has(el.tagName.toLowerCase()) ||
    el.closest("a,button,label,summary") !== null
  );
}

/** Root/layout containers that are never a meaningful annotation target. */
const NON_ANNOTATABLE: Record<string, true> = {
  html: true,
  body: true,
  main: true,
};
...
function readText(el: Element): string {
  return trimText(
    (el instanceof HTMLElement ? el.innerText : null) || el.textContent || "",
  );
}
...
function ancestorElement(node: Node): Element {
  return node.nodeType === 1 && node instanceof Element
    ? node
...
function rangeBoundary(node: Node, offset: number): TextRangeBoundary {
  ...
  while (cursor && cursor !== anchor && cursor.parentNode) {
    path.unshift(
      Array.prototype.indexOf.call(cursor.parentNode.childNodes, cursor),
    );
...
function showCard(opts: CardOptions): void {
  ...
  const rect = opts.range
    ? opts.range.getBoundingClientRect()
    : opts.hover!.getBoundingClientRect();
...
window.addEventListener("message", (event: MessageEvent<ChromeToSdk>) => {
  ...
  if (!msg || typeof msg.type !== "string" || !msg.type.startsWith("critique:"))
    return;
